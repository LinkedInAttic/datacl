...
// STOP FUNC DECL
{
  ...
  status = q_mmap(ddir_id, opfile, &cnt_X, &cnt_nX, 1);
  int *cntptr = (int *)cnt_X;
  assign_const_I4(cntptr, nR2, 0);
  long long chk_nR1 = 0;
  //-----------------------------------------------------------
  // f1 must be sorted ascending for this implementation
  status = int_get_meta(t1_id, f1_id, "sort_type", strbuf);
  if ( strcmp(strbuf, "ascending") == 0 ) { 
    offsets= (long long *)malloc(nR2 * sizeof(long long));
  ...
  status = add_fld(t2_id, cnt, ddir_id, opfile, I4, -1, &cnt_id, &cnt_rec); cBYE(status);
 BYE:
