...
#define BUFLEN 32
  ...
  status = mk_temp_file(opfile, (nR1 * sizeof(int)), &ddir_id);
  status = mk_temp_file(nn_opfile, (nR1 * sizeof(char)), &nn_ddir_id);

  status = q_mmap(ddir_id, opfile, &X, &nX, 1); cBYE(status);
  status = q_mmap(nn_ddir_id, nn_opfile, &nn_X, &nn_nX, 1); cBYE(status);
  /*-------------------------------------------------------- */
  bool is_fast = false;
  ...
  rs_munmap(X, nX);
  status = add_fld(t1_id, t1f2, ddir_id, opfile, I4, -1, &t1f2_id, &t1f2_rec);
  cBYE(status);
  if ( definitely_has_null_vals == true ) {
    /* No need to check */
    rs_munmap(nn_X, nn_nX);
    status = add_aux_fld(NULL, t1_id, NULL, t1f2_id, nn_ddir_id, nn_opfile, "nn", 
			 &nn_t1f2_id, &nn_t1f2_rec);
    cBYE(status);
  }
  else {
    status = par_f_to_s(nn_X, I1, NULL, 0, nR1, "sum", rslt_buf, BUFLEN);
    cBYE(status);
    ...
    if ( strcmp(Y[0], Y[1]) == 0 ) {
      ...
      status = add_aux_fld(NULL, t1_id, NULL, t1f2_id, nn_ddir_id, 
			   nn_opfile, "nn", &nn_t1f2_id, &nn_t1f2_rec);
      cBYE(status);
    }
  }
  /*-------------------------------------------------------*/
