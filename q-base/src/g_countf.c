...
// STOP FUNC DECL
{
  ...
  int fout_id = -1; GPU_REG_TYPE fout_rec;
  ...
  if ( strcmp(f1, fout) == 0 ) { go_BYE(-1); }
  if ( strcmp(cfld, f2) == 0 ) { go_BYE(-1); }
  if ( strcmp(cfld, fout) == 0 ) { go_BYE(-1); }
  if ( strcmp(f2, fout) == 0 ) { go_BYE(-1); }
  ...
  status = g_is_fld(gpu_reg, n_gpu_reg, fout, &fout_id, &fout_rec); cBYE(status);
  if ( fout_id < 0 ) { go_BYE(-1); }
  if ( is_fldtype_goodfor_gpu(fout_rec.fldtype) == false ) { go_BYE(-1); }


  status = g_chk_countf(f1_rec, cfld_rec, f2_rec, fout_rec);  cBYE(status);
  //----------------------------------------------------------
  char url[2048];     zero_string(url, 2048);
  char command[1024]; zero_string(command, 1024);
  sprintf(command, "f1f2opf3 d_fld1=%s d_cfld=%s d_fld2=%s d_out=%s",
        f1, cfld, f2, fout);
