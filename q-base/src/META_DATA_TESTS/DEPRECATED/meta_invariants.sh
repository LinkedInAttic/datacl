...
cd $TST_DATA_DIR
q dump _tbl.csv _fld.csv "mc_readable=[true]"
...
echo "name,SV,'is_dict_old=[false]:dict=[lkp_tbls]" >> _tbl_meta.csv
...
echo "name,SV,is_dict_old=[false]:dict=[lkp_flds]"         >> _fld_meta.csv
echo "fileno,I4,"       >> _fld_meta.csv

q dld Tflds _fld_meta.csv _fld.csv 'ignore_hdr=[true]'
cd -

echo "Meta Data Loaded by $0 in $PWD"
