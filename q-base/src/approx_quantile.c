...
// STOP FUNC DECL
{
  ...
  long long nR; int num_q; double eps;
  int out_val_ddir_id = -1, out_val_fileno = -1;

  if ( ( tbl == NULL ) || ( *tbl == '\0' ) ) { go_BYE(-1); }
  if ( ( fld == NULL ) || ( *fld == '\0' ) ) { go_BYE(-1); }
  if ( ( outtbl == NULL ) || ( *outtbl == '\0' ) ) { go_BYE(-1); }
  if ( ( str_num_q == NULL ) || ( *str_num_q == '\0' ) ) { go_BYE(-1); }
  if ( strcmp(tbl, outtbl) == 0 ) { go_BYE(-1); }
  status = stoI4(str_num_q, &num_q); cBYE(status);
  // TODO P4 Document below
  if ( ( num_q < 2 ) || ( num_q > 1024 ) )  { go_BYE(-1); } 
  if ( ( str_eps == NULL ) || ( *str_eps == '\0' ) ) { 
    eps = 0.001;
  }
  else {
    status = stoF8(str_eps, &eps); cBYE(status);
  ...
  status = add_tbl(outtbl, str_num_q, &outtbl_id, &outtbl_rec); cBYE(status);
  ...
  status = approx_quantile((int *)fld_X, (char *)cfld_X, nR, num_q, 
      eps, (int *)out_val_X, num_q, ptr_is_good);
  cBYE(status);
...
//----------------------------------------------------------------------------
{
  ...
  *ptr_estimate_is_good = -1; /* default */
  ...
  /* The Munro-Patterson algorithm assumes that the incoming data is
     in the form of packets of size k with sorted data. Since the
     sorting has to be done within each packet separately, we can
     parallelize this step as follows: we divide the incoming data
     into blocks of size NUM_THREADS*k (so that NUM_THREADS threads
     can be generated for each block and sorted separately in parallel */


  /* "inputPacket" is a 2d array of size NUM_THREADS * k: stores and
     sorts packets belonging to the same block in parallel.  Since the
     last packet might be incomplete, it will be dealt with
     separately, using "lastPacket" if k does not divide
     eff_siz. Variable last_packet_incomplete will be used to keep
     track of this */
  ...
  while ( current_loc_in_x < siz ) { 
    ...
    /* A block of data (containing NUM_THREADS packets, i.e.,
       NUM_THREADS * k integers) is processed inside this loop. For
       each packet, the following operations are done:
...
(3): If yes, copy the packet to a free buffer in the buffer array
using New() function. Else, use Collapse() function to merge two
buffers in the buffer array which have the same weight and free up one
buffer in the process (and copy the packet to that buffer)
    */
