...
// STOP FUNC DECL
{
  ...
  int f3_id = -1; GPU_REG_TYPE f3_rec;
  ...
  status = g_is_fld(gpu_reg, n_gpu_reg, f3, &f3_id, &f3_rec); cBYE(status);
  if ( f3_id < 0 ) { go_BYE(-1); }
  if ( is_fldtype_goodfor_gpu(f3_rec.fldtype) == false ) { go_BYE(-1); }

  status = g_chk_f1f2opf3(f1_rec, f2_rec, str_op, f3_rec);  cBYE(status);
  //----------------------------------------------------------
  char url[2048];     zero_string(url, 2048);
  char command[1024]; zero_string(command, 1024);
  sprintf(command, "f1f2opf3 d_fld1=%s d_fld2=%s op=%s d_fld3=%s",
        f1, f2, str_op, f3); 
