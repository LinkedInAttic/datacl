...
#define MAX_NUM_FLDS_TO_DMP 16
  ...
  if ( cfld_X == NULL ) { 
    ...
    status = f_to_s(tbl, cfld, "sum", str_rslt, 32); cBYE(status);
    // truncate string after colon
    for ( int i = 0; i < 32; i++ ) { 
      if ( str_rslt[i] == ':' ) { str_rslt[i] = '\0'; }
    }
    status = stoI8(str_rslt, &nn_nR); cBYE(status);
    if ( nn_nR <= 0 ) {
      fprintf(stderr, "WARNING! Nothing to dump\n"); goto BYE;
    }
  }
  size_t filesz = rec_size * nn_nR; 
  zero_string(cwd, (MAX_LEN_DIR_NAME+1));
  if ( getcwd(cwd, MAX_LEN_DIR_NAME) == NULL ) { go_BYE(-1); }
  if ( ( opdir == NULL ) || ( *opdir == '\0' ) ) {
    status = mk_file(cwd, cwd, opfile, filesz); cBYE(status);
  }
  else {
    status = mk_file(cwd, opdir, opfile, filesz); cBYE(status);
  }
  chdir(opdir);
