...
#define MAX_LEN 32
  ...
  nT = 32;
  long long block_size = aux_nR / nT;
  part_cntI4 = (int **)malloc(nT * sizeof(int *));
  for ( int tid = 0; tid < nT; tid++ ) { 
    part_cntI4[tid] = (int *)malloc(dst_nR * sizeof(int));
    // Initialize counters to 0 
#ifdef IPP
    ippsZero_32s(part_cntI4[tid], dst_nR); 
#else
    for ( int i = 0; i < dst_nR; i++ ) { part_cntI4[tid][i] = 0; }
#endif
  }
  ...
  for ( int tid = 0; tid < nT; tid++ ) { 
    int *tcntI4 = part_cntI4[tid];
    char *I1_src_ptr; short *I2_src_ptr; int *I4_src_ptr;
    int src_val;

    int out_lb = tid * block_size;
    int out_ub = out_lb + block_size;
    if ( tid == (nT-1) ) { out_lb = aux_nR; }

    for ( long long i = out_lb; i < out_ub; i++ ) { 
      switch ( aux_fldtype ) { 
