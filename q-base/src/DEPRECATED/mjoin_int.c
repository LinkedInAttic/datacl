	  ...
	  )
{
  ...
  int *ll_src_fld_ptr = NULL;
  int *dst_fld_val = NULL; char *nn_dst_fld_val = NULL;
  bool is_all_def = true; // assume no null values
  int src_marker;
  FILE *ofp = NULL, *nn_ofp = NULL;
  char *opfile = NULL, *nn_opfile = NULL;
  int dst_lnk_val, src_lnk_val, ll_src_fld_val;
  bool found;
  ...
  //--------------------------------------------------------
  status = collect_lnk_idx_vals_int(
  ...
  ll_src_fld_ptr = (int *)src_fld_X;
  dst_fld_val = (int *)malloc(dst_nR * sizeof(int));
  ...
  fwrite(dst_fld_val, dst_nR, sizeof(int), ofp);
