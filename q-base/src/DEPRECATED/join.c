      ...
      )
{
  ...
  sprintf(str_meta_data, "fldtype=%s:n_sizeof=4:filename=",
	src_fld_meta.fldtype);
  if ( strcmp(src_lnk_meta.fldtype, "int") == 0 ) { 
    ...
    status = join_int( src_fld_X, src_lnk_X, dst_lnk_X, nn_src_fld_X,
	  nn_src_lnk_X, nn_dst_lnk_X, src_nR, dst_nR, &is_any_null,
	  str_meta_data, &nn_opfile);
    cBYE(status);
  }
    ...
    status = join_longlong( src_fld_X, src_lnk_X, dst_lnk_X, nn_src_fld_X,
	  nn_src_lnk_X, nn_dst_lnk_X, src_nR, dst_nR, &is_any_null,
	  str_meta_data, &nn_opfile);
    cBYE(status);
  }
  ...
  /* LATER: FOR STRINGS JOINING 
  if ( sz_opfile != NULL ) { 
    status = add_aux_fld(docroot, db, dst_tbl, dst_fld, sz_opfile, "sz");
    cBYE(status);
  }
  else {
    unlink(sz_opfile);
