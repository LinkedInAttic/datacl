...
// STOP FUNC DECL
{
  ...
  cilkfor ( int tid = 0; tid < nT; tid++ ) { 
    ...
    for ( long long i = lb; i < ub; i++ ) { 
      unsigned __ITYPE__ inval = (unsigned __ITYPE__) X[i];
      int bin = inval >> shift;
  ...
  */
  ...
  cilkfor ( int tid = 0; tid < nT; tid++ ) { 
    __ITYPE__ *lY = (__ITYPE__ *)Y; lY += bin_offset[tid];
    if ( cnt[tid] == 0 ) { continue; }
    if ( strcmp(mode, "asc") == 0 ) {
      qsort_asc___XTYPE__(lY, cnt[tid], sizeof(__ITYPE__), NULL);
    }
    else if ( strcmp(mode, "dsc") == 0 ) {
      qsort_dsc___XTYPE__(lY, cnt[tid], sizeof(__ITYPE__), NULL);
    }
...
#ifdef UNIT_TEST
/*
gcc -g -std=gnu99 -Wall auxil.c mmap.c _x.c ./AUTOGEN/GENFILES/assign_I*.c ./AUTOGEN/GENFILES/qsort_*__XTYPE__.c -I./AUTOGEN/GENFILES -I. -lm
*/
#include <stdio.h>
#include <sys/mman.h>
#include <stdbool.h>
...
#include "mmap.h"
#include "auxil.h"
#include "srt_uniform___XTYPE__.h"
int
main(
    int argc,
    char **argv
    )
{
  ...
  status = srt_uniform___XTYPE__((int *)X, nR, (int *)Y, "asc"); cBYE(status);
