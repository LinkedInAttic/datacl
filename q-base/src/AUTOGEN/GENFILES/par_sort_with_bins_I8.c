...
// STOP FUNC DECL
{
  ...
  for ( int tid = 0; tid < nT; tid++ ) {
    ...
    for ( long long i = lb; i < ub; i++ ) {
      ...
      // long long b;
      // status = find_smallest_geq_I8(lb, num_bins, inval, &b); 
      int b;
      status = get_bin_I8(inval, binlb, binub, num_bins, &b);
      bin_idx[i] = b;
      bin_cnt[b]++;
    }
  }
...
#pragma omp parallel for
  for ( int b = 0; b < num_bins; b++ ) {
    ...
    qsort_asc_I8(X, bin_cnt[b], sizeof(I8), NULL); 
  }
...
{
  int status = 0;
  long long *X = NULL, *Y = NULL;
  long long *binlb = NULL, *binub = NULL;
