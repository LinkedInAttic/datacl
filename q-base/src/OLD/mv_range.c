...
// STOP FUNC DECL
{
  ...
  *cptr2++ = '\0'; /* null terminate */
  ...
  // Create output
  status = open_temp_file(&ofp, &opfile, src_nR*src_fld_meta->n_sizeof); cBYE(status);
  fclose_if_non_null(ofp);
  status = mk_file(opfile, src_nR * src_fld_meta->n_sizeof); cBYE(status);
  status = rs_mmap(opfile, &dst_fld_X, &dst_fld_nX, 1); cBYE(status);
  //------------------------------------------------------
