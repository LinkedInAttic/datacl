...
static long long g_nR = INT_MIN; /* number of rows to process */
static int   g_f1type = -1;
static char *g_f1_X = NULL;
static char *g_nn_f1_X = NULL;
static char *g_op_X = NULL; /* where to write output */
static char *g_op = NULL;   /* operation to be performed */
static char *dst_fld_type = NULL;
static char *date_format = NULL;

/* Following for case when f1 = string */
static long long *offset_X = NULL;
static char *g_sz_f1_X = NULL;
/*-------------------------------------------*/
		  ...
		  )
{
  ...
  for ( int t = 0; t < nT; t++ ) {
    if ( t != tid ) { continue; }
    status = get_outer_lb_ub(nR, nT, t, &lb, &ub);
    cBYE(status);
    num_rows_this_thread = ub - lb;
    ...
    */
	...
	/*--------------------------------------------------*/
	/* We use sz instead of nn */
	sz_g1_X = (int *)sz_f1_X;
	sz_g1_X += lb;
	dateconv(g1_X, (int *)sz_g1_X, nX, date_format,(int *)my_op_X);
      }
      else {
	go_BYE(-1);
      }
...
// STOP FUNC DECL 
{
  ...
  char *cum_X = NULL; size_t cum_nX = 0 ; /* for offset for string */
  /*---------------------------------------------*/
  ...
  if ( in_nn_f1_X == NULL ) {
    ...
    status = open_temp_file(&nn_ofp, &nn_opfile, nR); cBYE(status);
...
#define MIN_ROWS_FOR_PARALLELISM_F1OPF2 65536
