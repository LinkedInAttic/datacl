	  ...
	  )
{
  ...
  for ( int i = 0; i < nn_dst_nR; i++ ) {
    ...
    if ( found == true ) { 
      ...
      for ( ; src_marker < src_nR; ) {
	if ( src_pair[src_marker].lnk_val == dst_lnk_val ) {
	  src_marker++;
	}
	else if ( src_pair[src_marker].lnk_val < dst_lnk_val ) {
	  goto_BYE(-1);
	}
	else { 
	  break;
  ...
  /* ll_src_fld_ptr = XXXXXXXXXXXXXXXX */

  fwrite(dst_fld_val, dst_nR, sizeof(TEXT1), ofp);
  fclose_if_non_null(ofp); // primary field

  /* Check if nn field is needed. Delete if not */
  is_any_null = false;
  for ( int i = 0; i < dst_nR; i++ ) {
    if ( nn_dst_fld_val[i] == FALSE ) { 
      is_any_null = true;
      break;
    }
  }
...
collect_lnk_idx_vals_TEXT2(
      ...
      )
  {
    ...
    int nn_src_nR, nn_dst_nR;
    TEXT3 *src_pair = NULL, *dst_pair = NULL;
  int rec_size = sizeof(TEXT1) + sizeof(int);
  ...
  cBYE(status);
  qsort(temp_src_X, nn_src_nR, rec_size, sort_TEXT6_aa_compare);
