...
update_counter(cntr_id,cntr_freq,cntr_siz,ptr_active_cntr_siz,bf_id,bf_freq,bf_siz)
: This function updates the counter array (cntr_id, cntr_freq) by
adding new id and freq data specified by (bf_id, bf_freq).
...
cntr_siz: Total number of counters ( size of (cntr_id,cntr_freq) )
available to do the counting.

ptr_active_cntr_siz: Number of counters in the (cntr_id, cntr_freq)
counter array which are currently being used by some ids
(active_cntr_siz).
...
//--------------------------------------------------------------------------
{
  ...
  /* (temp_cntr_id,temp_cntr_freq) stores the merged and sorted
     (sorted in id) data of the counters (cntr_id,cntr_freq) and
     (bf_id, bf_freq) */
