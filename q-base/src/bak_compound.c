...
// STOP FUNC DECL
{
  ...
  if ( strcmp(op, "s_to_f"    ) == 0 ) { xop = op_s_to_f; }
...
// STOP FUNC DECL
{
  ...
  *ptr_has_null = true;
  ...
  if ( ( fld == NULL ) || ( *fld == '\0' ) ) { go_BYE(-1); }
  if ( ( fld_idx < 0 ) || ( fld_idx > 2 ) ) { go_BYE(-1); }
  int len = strlen(fld);
  ...
  for ( int lno = 0; lno < n_comp_expr ; lno++ ) {
	...
	if ( f2_has_null == true ) { go_BYE(-1); }
      ...
      status = proc_fld(tbl_id, nR, fld2, jnk_fldtype, jnk_bool, 
			lno, 1, 0, comp_expr, &f2_fldtype, &f2_has_null); 
      cBYE(status);
      if ( ( f2_fldtype != I4 ) && ( f2_fldtype != I8 ) ) { go_BYE(-1); }
      //---------------------------------------------------
      status = read_nth_val(line, delim, colidx++, fld3, BUFSZ);
	...
	f3_has_null = false;
      ...
      fprintf(stderr, "TODO P1: NOT IMPLEMENTED\n"); go_BYE(-1); 
    }
    else if ( strcmp(qfn, "countf") == 0 ) {
      fprintf(stderr, "TODO P1: NOT IMPLEMENTED\n"); go_BYE(-1); 
    }
    else if ( strcmp(qfn, "s_to_f") == 0 ) {
      fprintf(stderr, "TODO P1: NOT IMPLEMENTED\n"); go_BYE(-1); 
    }
...
// STOP FUNC DECL
{
  ...
  if ( ( f.is_temp == 0 ) && ( X != NULL ) ) { 
    if ( ftype == B ) { 
      if (    X != NULL ) { X    += (fldsz * (lb/8)); }
    }
    else {
      status = get_fld_sz(ftype, &fldsz);  cBYE(status);
      if (    X != NULL ) { X    += (fldsz * lb); }
    }
    status = get_fld_sz(I1 , &fldsz);  cBYE(status);
    if ( nn_X != NULL ) { nn_X += (fldsz * lb); }
  }
  *ptr_X = X;
  *ptr_nn_X = nn_X;
 BYE:
...
// STOP FUNC DECL
{
  ...
  double l_numerF8; long long l_numerI8, l_denomI8;
  double     numerF8[MAX_STATEMENTS_IN_COMP_EXPR];
  long long  numerI8[MAX_STATEMENTS_IN_COMP_EXPR];
  long long  denomI8[MAX_STATEMENTS_IN_COMP_EXPR];
  bool  is_numer_set[MAX_STATEMENTS_IN_COMP_EXPR];
  //------------------------------------
  COMP_EXPR_FLD_INFO_TYPE f1, f2, f3, f4;
  FLD_TYPE f1type, f2type, f3type, f4type;
#define BUFLEN 32
#define RSLT_BUF_SIZE 1024
  ...
  for ( int b = 0; b < num_blocks; b++ ) {
    ...
    for ( int i = 0; i < n_comp_expr; i++ ) { 
      ...
      char *f1_X = NULL, *nn_f1_X = NULL;
      char *f2_X = NULL, *nn_f2_X = NULL;
      char *f3_X = NULL, *nn_f3_X = NULL;
      char *f4_X = NULL, *nn_f4_X = NULL;
      ...
      status = offset_pointers(comp_expr, i, 0, lb, &f1_X, &nn_f1_X); 
      cBYE(status);
      status = offset_pointers(comp_expr, i, 1, lb, &f2_X, &nn_f2_X); 
      cBYE(status);

      f3 = comp_expr[i].fld[2];
      f3type  = f3.fldtype;
      f3_X    = f3.X;
      nn_f3_X = f3.nn_X; 
      switch ( comp_expr[i].op ) { 
      case op_count : 
	/* Do not increment pointer to f3 which is actually cnt in
	 * outtbl. Also, if first iteration, then you need to
	...
	break; 
      case op_f1f2opf3 : 
	if ( ( f3.is_temp == 0 ) && ( f3_X != NULL ) ) { 
	  if ( f3type == B ) { 
	    if (    f3_X != NULL ) { f3_X    += (fldsz * (lb/8)); }
	  }
	  else {
	    status = get_fld_sz(f3type, &fldsz);  cBYE(status);
	    if ( f3_X != NULL ) { f3_X    += (fldsz * lb); }
	  }
	  status = get_fld_sz(I1    , &fldsz);  cBYE(status);
	  if ( nn_f3_X != NULL ) { nn_f3_X += (fldsz * lb); }
	}
	break;
      default : 
	...
	break;
      ...
      f4 = comp_expr[i].fld[3];
      f4type  = f4.fldtype;
      f4_X    = f4.X;
      nn_f4_X = f4.nn_X; 
      switch ( comp_expr[i].op ) { 
      case op_countf : 
	/* Do not increment pointer to f4 which is actually cnt in
	 * outtbl. Also, if first iteration, then you need to
	 * initialize counts to 0 */
	...
	break;
      default : 
	/* there is no f3 field for these operation */
	break;
      }
	...
	cBYE(status);
	status = stoI8(buf1, &l_denomI8); cBYE(status); 
	if ( l_denomI8 < 0 ) { go_BYE(-1); }
	if ( l_denomI8 > 0 ) {
	  denomI8[i] += l_denomI8;

	  switch ( f1type ) {
	  case I1 : case I2 : case I4 : case I8 : 
	    status = stoI8(buf0, &l_numerI8); cBYE(status); 
		...
		numerI8[i] = min(numerI8[i], l_numerI8);
	      }
	      else if ( strcmp(str_op_spec, "max") == 0 ) { 
		numerI8[i] = max(numerI8[i], l_numerI8);
	      }
	    ...
	    break;
	  case F4 : case F8 : 
	    status = stoF8(buf0, &l_numerF8); cBYE(status); 
	    if ( is_numer_set[i] == false ) {
	      is_numer_set[i] = true; 
	      numerF8[i] = l_numerF8;
	    }
	      ...
	      if ( strcmp(str_op_spec, "min") == 0 ) { 
		numerF8[i] = min(numerF8[i], l_numerF8);
	      }
	      else if ( strcmp(str_op_spec, "max") == 0 ) { 
		numerF8[i] = max(numerF8[i], l_numerF8);
	      }
	      else if ( strcmp(str_op_spec, "sum") == 0 ) { 
		numerF8[i] += l_numerF8;
	      }
  ...
  for ( int i = 0; i < n_comp_expr; i++ ) { 
	...
	f1 = comp_expr[i].fld[0];
	f1type  = f1.fldtype;
	switch ( f1type ) { 
	case I1 : case I2 : case I4 : case I8 : 
	  fprintf(stdout, "%s=%lld:%lld\n", comp_expr[i].env_var, numerI8[i], denomI8[i]);
	  break;
	case F4 : case F8 : 
	  fprintf(stdout, "%s=%lf:%lld\n", comp_expr[i].env_var, numerF8[i], denomI8[i]);
	  break;
	default : 
	  go_BYE(-1);
	  break;
