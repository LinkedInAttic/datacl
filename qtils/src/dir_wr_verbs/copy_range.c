...
#define BUFLEN (MAX_LEN_FLD_NAME+1+32)
  ...
  if ( is_mat_range ) { 
    ...
    if ( ( mat_tbl != NULL ) && ( *mat_tbl != '\0' ) ) {
      ...
      if ( mat_fld_rec.sort_type != ascending ) { go_BYE(-1); }
      is_mat_tbl = true;
    }
  }
  //-=======================================================
  ...
  status = q_mmap(ddir_id, opfile, &dst_X, &dst_nX, 1); cBYE(status);

  if ( nn_src_fld_id >= 0 ) { 
    status = mk_temp_file(nn_opfile, (sizeof(char) * dst_nR), &nn_ddir_id); cBYE(status);
    status = q_mmap(nn_ddir_id, nn_opfile, &nn_dst_X, &nn_dst_nX, 1); cBYE(status);
  }
#define CUTOFF 32
  /* If 
  ...
  if ( is_simple_copy ) {
    ...
    if ( mat_dst_nR < dst_nR ) {
      ...
      q_trunc(ddir_id, opfile, (dst_nR * fldsz));
    }
  }
  else { go_BYE(-1); }
  ...
  if ( nn_src_fld_id >= 0 ) { 
    ...
    status = ext_f_to_s(dst_tbl, strbuf, "sum", strbuf, BUFLEN ); cBYE(status); 
    status = explode(strbuf, ':', &Y, &nY); cBYE(status);
    if ( nY != 2 ) { go_BYE(-1); }
    if ( strcmp(Y[0], Y[1]) == 0 ) { /* nn field not needed */
      status = drop_nn_fld(dst_tbl, dst_fld); cBYE(status);
    }
  }
  //--------------------------------------------
  if ( ( order != NULL ) && ( strcmp(order, "sortA") == 0 ) ) { 
    status = ext_f_to_s(dst_tbl, dst_fld, "is_sorted", strbuf, BUFLEN);
    cBYE(status);
