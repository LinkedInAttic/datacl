...
// STOP FUNC DECL
{
  ...
  char *lb_X = NULL; size_t lb_nX = 0; int lb_ddir_id; 
  char *ub_X = NULL; size_t ub_nX = 0; int ub_ddir_id; 
  ...
  char lbfile[MAX_LEN_FILE_NAME+1];
  char ubfile[MAX_LEN_FILE_NAME+1];
  //----------------------------------------------------------------
  zero_string(lbfile, MAX_LEN_FILE_NAME+1);
  zero_string(ubfile, MAX_LEN_FILE_NAME+1);
  ...
  status = mk_temp_file(lbfile, filesz, &lb_ddir_id); cBYE(status);
  status = q_mmap(lb_ddir_id, lbfile, &lb_X, &lb_nX, true); cBYE(status);
  long long *lbI8 = (long long *)lb_X;

  status = mk_temp_file(ubfile, filesz, &ub_ddir_id); cBYE(status);
  status = q_mmap(ub_ddir_id, ubfile, &ub_X, &ub_nX, true); cBYE(status);
  ...
  if ( dst_nR < src_nR ) { 
    q_trunc(lb_ddir_id, lbfile,  (dst_nR * sizeof(long long)));
    q_trunc(ub_ddir_id, ubfile,  (dst_nR * sizeof(long long)));
  }
  ...
  status = add_fld(dst_tbl_id, lb_fld, lb_ddir_id, lbfile, I8, -1, 
      &lb_fld_id, &lb_fld_rec); cBYE(status);
  status = add_fld(dst_tbl_id, ub_fld, ub_ddir_id, ubfile, I8, -1,
