...
// STOP FUNC DECL
{
  ...
  FILE *dst_ofp = NULL; char dst_opfile[MAX_LEN_FILE_NAME+1];
  int t1_id, t2_id; 
  int f1_id, nn_f1_id; 
  int f2_id, nn_f2_id; 
...
#define BUFSZ 16
  ...
  if ( b_write_dst ) { 
    ...
    status = mk_temp_file(dst_opfile, (nR1 * fldsz), &dst_ddir_id); cBYE(status);
    status = q_mmap(dst_ddir_id, dst_opfile, &dst_f1_X, &dst_f1_nX, 1); // writing 
  }
  //--------------------------------------------------------
  ...
  if ( ( cfld != NULL ) && ( *cfld != '\0' ) ) {
    ...
    case I1 : case I2 : case F4 : case F8 : 
      ...
      break;
    case I4 : 
      status = add_fld(t1_id, dst_f1, ddir_id, dst_opfile, I4, -1, &dst_f1_id, &dst_f1_rec);
      cBYE(status);
      break;
    case I8 : 
      status = add_fld(t1_id, dst_f1, ddir_id, dst_opfile, I8, -1, &dst_f1_id, &dst_f1_rec);
      cBYE(status);
      break;
    default : 
      go_BYE(-1);
    ...
    status = add_aux_fld(NULL, t1_id, NULL, dst_f1_id, dst_ddir_id, opfile, 
